...
public class LojasLogic : ILojasService
{
    ...
    public Task<LojasEntity?> ObterAsync(int id) => _repo.ObterAsync(id);

    public async Task<LojasEntity> CriarAsync(LojasEntity input)
    {
        var loja = Sanitizar(input);
    ...
    public async Task<LojasEntity> AtualizarAsync(int id, LojasEntity input)
    {
        _ = await _repo.ObterAsync(id) ?? throw new KeyNotFoundException("Loja não encontrada.");
        ...
        if (!string.IsNullOrWhiteSpace(loja.Cnpj))
        {
            ...
            if (loja.Cnpj!.Length != 14)
                throw new ArgumentException("CNPJ deve conter 14 dígitos.");
            if (await _repo.CnpjExisteAsync(loja.Cnpj, ignoreId: id))
    ...
    public async Task<bool> RemoverAsync(int id)
    {
        if (await _repo.TemDependenciasAsync(id))
            throw new InvalidOperationException("Não é possível excluir: há dados vinculados (produtos/funcionários/clientes).");

        return await _repo.RemoverAsync(id);
    }

    // helpers
    private static LojasEntity Sanitizar(LojasEntity e) => new()
    {
